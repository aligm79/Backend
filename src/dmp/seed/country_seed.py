"""The ~195 ISO 3166-1 countries (alpha-2 code + English name), ported verbatim from
CountrySeedData.cs. Flag emojis are derived from the code via regional-indicator pairs.
"""

from __future__ import annotations

# (ISO alpha-2 code, English name) — alphabetical by name, matching the .NET seed.
...
def flag_from_code(code: str) -> str:
    """Convert an ISO alpha-2 code to its flag emoji via regional-indicator pairs."""
    if len(code) != 2:
        return ""
    upper = code.upper()
    if not (upper[0].isalpha() and upper[1].isalpha()):
        return ""
    offset = 0x1F1E6 - ord("A")
    return chr(ord(upper[0]) + offset) + chr(ord(upper[1]) + offset)
